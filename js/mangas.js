...
// Complétion auto
for (const g of genresMangas) {
  const k = normalizeGenreKey(g);
  if (!Object.prototype.hasOwnProperty.call(genreWeightsMangas, k)) {
    genreWeightsMangas[k] = DEFAULT_GENRE_WEIGHT;
  }
}
...
export function weightForGenre(name) {
  ...
  return Object.prototype.hasOwnProperty.call(genreWeightsMangas, k)
    ? Number(genreWeightsMangas[k])
...
export function bestPrimaryFromGenres(it) {
  ...
  // trie par priorité numérique croissante
...
/* ==========================================================
 * 5) Boot de la page mangas (déplacé depuis mangas.html)
 * ========================================================== */
...
export async function initMangasPage() {
  // 1) Flèche/panneau (commun)
  ...
  // 3) Auth + data + explorer
  let RAW = [];

  const boot = async (userKey) => {
    const oeuvres = await chargerDonneesCategorie('mangas');
    const mangas = oeuvres.filter(estManga);
    RAW = mangas;

    // TomSelect (si présents)
    try {
      const mkTS = (sel) => {
        ...
        return new TomSelect(sel, {
          ...
          options: genresMangas.map(g => ({ value: g, text: g }))
        });
      };
      mkTS('#genresIn'); mkTS('#genresOut');
    } catch (e) { /* panneau replié au chargement → OK */ }
    ...
    initExplorer({
      ...
      render: (arr) => afficherCartes(arr, 'mangas', 'mangaCards', userKey)
    });
  };
