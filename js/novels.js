...
// 1) Genres (reprend large, proche mangas mais tu peux limiter)
export const genresNovels = [
...
// 3) Poids
export const DEFAULT_GENRE_WEIGHT_NOVEL = 6.0;
export const genreWeightsNovels = {
  // Leads spéciaux
  "female lead": 0,
  "male lead": 0,
  "autre": 0,
...
// Complétion auto
for (const g of genresNovels) {
  const k = normalizeGenreKey(g);
  if (!Object.prototype.hasOwnProperty.call(genreWeightsNovels, k)) {
    genreWeightsNovels[k] = DEFAULT_GENRE_WEIGHT_NOVEL;
  }
}

// 4) Helpers
export function weightForGenreNovel(name) {
  const k = normalizeGenreKey(name);
  return Object.prototype.hasOwnProperty.call(genreWeightsNovels, k)
    ? Number(genreWeightsNovels[k])
    : DEFAULT_GENRE_WEIGHT_NOVEL;
}
...
export async function initNovelsPage() {
  ...
  const boot = async (userKey) => {
    const oeuvres = await chargerDonneesCategorie('novels');
    ...
    try {
      const mkTS = (sel) => {
        ...
        return new TomSelect(sel, {
          ...
          options: genresNovels.map(g => ({ value: g, text: g }))
        });
      };
    ...
    initExplorer({
      ...
      genres: genresNovels,
      userKey,
      getWeight: (k) => weightForGenreNovel(k),
      mount: {
        titleQ: '#qTitle',
        descQ: '#qDesc',
        inSel: '#genresIn',
        outSel: '#genresOut',
        status: '#userStatus',  // même filtres que mangas (progression etc.)
        minCh: '#minCh',
        maxCh: '#maxCh',
        sortBy: '#sortBy',
        sortDir: '#sortDir',
        ...
        apply: '#applyBtn',
        reset: '#resetBtn',
        gridId: 'novelCards'    // ⚠️ correspond à novels.html
      },
      render: (arr) => afficherCartes(arr, 'novels', 'novelCards', userKey)
    });
  };
